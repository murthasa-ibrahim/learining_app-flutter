...
class ExcerciseServices {
  Future<ExcercisesModel> getExcercise(int testId) async {
    const url = ApiUrls.baseUrl + ApiUrls.exercises;
    ...
    try {
      ...
      final response =
          await dio.post(url, data: {"category": "IELTS", "test_id":testId});


      if (response.statusCode! >= 200 || response.statusCode! <= 299) {
        log(response.data.toString());
        return excercisesModelFromJson(jsonEncode(response.data));
      } else {
